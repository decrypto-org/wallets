...
A transaction description is comprised of the address of the recipient and an amount of cryptocurrency. The funding process consists of the following steps:

\begin{itemize}
    \item Filling in a valid address corresponding to a public key that (a) holds more cryptocurrency in its balance than what will be spent in the transaction and (b) is derivable from the seed.
    \item Filling in the valid nonce in order to make this transaction spendable.
    \item Signing the transaction with the appropriate secret key.
\end{itemize}
