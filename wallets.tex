...
An input is valid with regards to a state $\btcstate$ if (a) the output it references exists and is not already spent and (b) the unlocking script of the input ``unlocks'' the locking script of the output. In the case of Bitcoin, the unlocking script runs on an empty stack, and subsequently the locking script runs on the resulting stack of the unlocking script's execution. The input is considered to unlock the output if the locking script terminates successfully.
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        \Function{$\btcapply$}{$\btcstate, tx$}
            ...
            \EndFor
            \For{$i \in tx.\inputs$}
                \State{$\btcstate.\spentOutputs \cupeq \{i.\mathtt{output}\}$}
            \EndFor
...
\begin{itemize}
    ...
    \[
        \ethstate[\sender].\balance \ge \amount+\fee
    \]
\end{itemize}
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        \Function{$\ethapply$}{$\ethstate, tx$}
            ...
            \State{$\ethstate[tx.\sender].\balance \minuseq tx.\amount$}
            \State{$\ethstate[tx.\receiver].\balance \pluseq tx.\amount$}
